...
- **Now at:** find and fix the `ppu_lifter` guest-frame overflow in the shader-generation chain (write-watch covering byte-writes + memcpy on `func_000C3B18`'s back-chain and saved-`r31` slot). That unblocks all Cg handles → shader binding → `demo_draw`. **The demo does not yet render its own content** — everything up to shader compilation works; this systemic corruption is the sole remaining blocker.
